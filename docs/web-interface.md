...
| `network/wireless/config/uncombine`: SSID, encryption, channel/width, power, station limit, hidden/isolation settings | Wireless section records with raw exports and a limited typed interface model exist | Map radio/section fields and derived channel controls; structured records do not establish full field-schema coverage |
